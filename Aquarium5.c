...
void toggle_fullscreen() {
    ...
    // Queue a redraw to update background
    if (drawing_area) {
        gtk_widget_queue_draw(drawing_area);
    }
}
...
void update_stats() {
    ...
    char stats[1024];  // Increased buffer size
...
int main(int argc, char *argv[]) {
    ...
    win->title = "🐠 AQUARIUM STUDIO - Dynamic Fullscreen";
    ...
    create_normal_btn->label = "🐟 CREATE NORMAL FISH";
    ...
    create_predator_btn->label = "🦈 CREATE PREDATOR";
    ...
    selected_label->text = "⚙️ SELECTED FISH:";
    ...
    tips_label->text = "💡 HOW TO PLAY:\n"
                       ...
                       "• HOVER mouse over fish = FLEE!\n"
                       "• Regroup after 2 seconds\n"
                       "• Right-click = Feed fish\n"
                       "• Drag = Move camera\n"
                       "• Scroll = Zoom\n"
